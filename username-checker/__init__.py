...
# use export TWITTER_TOKEN="YOUR_TOKEN" to set the token
    ...
    for username_chunk in username_chunks:
        ...
        if "data" in data:
            for user in data["data"]:
                ...
                threshold = parser.parse(f"{int(datetime.now().year) - YEARS_INACTIVE}-01-01")
                ...
                if (
                    ...
                    logging.info(f"ACTIVE {username}")
                    continue
                ...
                if "meta" not in tweet_data:
                    ...
                    logging.info(f"PRIVATE {username}")
                    continue

                if tweet_data["meta"]["result_count"] > 0:
                    print(f"\033[1;31mACTIVE\033[0m\t\t{username}")
                    logging.info(f"ACTIVE {username}")
                    # here something to return true or false for azure
                else:
                    print(f"\033[1;33mINACTIVE\033[0m\t{username}")
                    logging.info(f"INACTIVE {username}")
    return available_users

def check_users(usernames):
    for username in usernames:
        response = requests.get(
